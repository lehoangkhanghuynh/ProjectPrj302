    ...
    <head>
        ...
        <style>
            ...
            *, *::before, *::after {
            ...
            .dist-bar-fill {
                height: 100%;
                background: linear-gradient(90deg, var(--gold), #E6B830);
                border-radius: 4px;
            }
            .dist-count {
                font-size: 0.72rem;
            ...
            .modal-content {
                border-radius: 20px;
                border: none;
                box-shadow: 0 24px 64px rgba(108,63,197,0.2);
            }
            .modal-header {
    ...
    <body>
        ...
        <nav class="navbar-main" style="position:relative;">
            ...
            <ul class="nav-links">
                ...
                <li><a href="${pageContext.request.contextPath}/instructors.jsp">Giảng viên</a></li>
                    ...
                    <li><a href="${pageContext.request.contextPath}/adminController?action=dashboard">Administrator Manager</a></li>
                    </c:if>
                    <c:if test="${sessionScope.user != null && sessionScope.user.role == 2}">
                    <li><a href="${pageContext.request.contextPath}/instructor/instructorDashboard.jsp">Instructor Manager</a></li>
                    </c:if>
        ...
        <div class="course-hero">
            <div class="hero-inner">
                ...
                <div class="hero-info">
                    <a href="${pageContext.request.contextPath}/courseController?action=ExploreCourse" class="btn-back">
                    ...
                    <%-- NÚT HÀNH ĐỘNG — FIX: lesson? → courseController?action=lesson& --%>
                    <c:choose>
                        <c:when test="${empty sessionScope.user}">
                            <a href="${pageContext.request.contextPath}/login.jsp" class="btn-enroll-hero">
                        ...
                        <c:when test="${IS_COMPLETED}">
                            ...
                            <a href="${pageContext.request.contextPath}/courseController?action=lesson&courseId=${COURSE.courseId}" class="btn-enroll-hero">
                        ...
                        <c:when test="${IS_ENROLLED}">
                            ...
                            <a href="${pageContext.request.contextPath}/courseController?action=lesson&courseId=${COURSE.courseId}" class="btn-study-hero">
                                <i class="bi bi-play-circle-fill"></i> Vào học ngay
                            </a>
                        </c:when>
                        <c:otherwise>
                            <button class="btn-enroll-hero" data-bs-toggle="modal" data-bs-target="#enrollModal">
            ...
            <c:if test="${REVIEW_COUNT > 0}">
                <div class="rating-summary-card">
                    ...
                    <div class="rating-dist">
                        <c:forEach var="row" items="${DIST_LIST}">
                            <div class="dist-row">
                                <span class="dist-label">${row.star} <i class="bi bi-star-fill" style="color:var(--gold);font-size:0.6rem;"></i></span>
                                <div class="dist-bar-wrap"><div class="dist-bar-fill" style="width:${row.pct}%;"></div></div>
                        ...
                        <c:forEach var="rv" items="${REVIEWS}">
                            <div class="review-item">
                                <div class="review-header">
                                    <div class="review-avatar">${fn:substring(rv.fullname, 0, 1)}</div>
                                    <div class="review-meta">
                                        <div class="review-name">${rv.fullname}</div>
                                        <div class="review-date">
                                            <i class="bi bi-calendar3"></i>
                                            <c:if test="${rv.createdAt != null}">${fn:substring(rv.createdAt.toString(), 0, 10)}</c:if>
                                            </div>
                                        </div>
                    ...
                    <div class="modal-body">
                        ...
                        <c:if test="${not empty sessionScope.user}">
                            <div class="modal-info-row">
                                ...
                                <span class="modal-info-value balance-val"><fmt:formatNumber value="${sessionScope.user.balance}" type="number"/> ₫</span>
                            </div>
                            ...
                            <c:if test="${afterBalance < 0}">
                                <div style="background:#FFF3F3;border:1px solid #FFCDD2;border-radius:8px;padding:10px 14px;margin-top:10px;font-size:0.82rem;color:#C62828;font-weight:600;">
                                    ...
                                    Số dư không đủ! <a href="${pageContext.request.contextPath}/mainController?action=payment" style="color:#C62828;font-weight:700;">Nạp tiền ngay →</a>
                                </div>
                            </c:if>
                        </c:if>
                    </div>
                    <div class="modal-footer">
                        ...
                        <c:choose>
                            <c:when test="${not empty sessionScope.user && sessionScope.user.balance >= COURSE.fee}">
                                <form action="${pageContext.request.contextPath}/courseController" method="post" style="margin:0;">
