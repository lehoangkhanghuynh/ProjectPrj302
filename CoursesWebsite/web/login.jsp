...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* NAVBAR */
        .navbar-main {
            background: var(--purple-deep); padding: 0 48px; height: 64px;
            display: flex; align-items: center; justify-content: space-between;
            position: sticky; top: 0; z-index: 50;
            box-shadow: 0 2px 16px rgba(0,0,0,0.2);
        }
        .brand { font-family: 'Playfair Display', serif; font-size: 1.5rem; font-weight: 700; color: #fff; text-decoration: none; }
        .brand span { color: var(--gold); }
        .nav-links { display: flex; gap: 4px; list-style: none; }
        .nav-links a { font-size: 0.875rem; font-weight: 500; color: rgba(255,255,255,0.7); text-decoration: none; padding: 7px 13px; border-radius: 6px; transition: all 0.15s; }
        .nav-links a:hover { background: rgba(255,255,255,0.08); color: #fff; }
        .nav-right { display: flex; align-items: center; gap: 10px; }
        .btn-nav-login { font-size: 0.875rem; font-weight: 600; color: rgba(255,255,255,0.85); background: none; border: none; padding: 7px 16px; border-radius: 6px; cursor: pointer; font-family: 'DM Sans', sans-serif; transition: color 0.15s; }
        .btn-nav-login:hover { color: #fff; }
        .btn-nav-join { font-size: 0.875rem; font-weight: 700; background: var(--gold); color: var(--purple-deep); border: none; padding: 8px 20px; border-radius: 8px; cursor: pointer; transition: opacity 0.15s; font-family: 'DM Sans', sans-serif; }
        .btn-nav-join:hover { opacity: 0.88; }
        .user-chip { display: flex; align-items: center; gap: 9px; background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.15); border-radius: 8px; padding: 5px 12px; cursor: pointer; transition: background 0.15s; position: relative; }
        .user-chip:hover { background: rgba(255,255,255,0.14); }
        .u-avatar { width: 30px; height: 30px; border-radius: 50%; background: linear-gradient(135deg, var(--purple), var(--gold)); display: flex; align-items: center; justify-content: center; font-size: 0.8rem; font-weight: 700; color: #fff; }
        .u-name { font-size: 0.85rem; font-weight: 600; color: #fff; }
        .user-dropdown { position: absolute; top: 48px; right: 0; background: #fff; border: 1px solid var(--border); border-radius: 10px; padding: 6px; min-width: 180px; box-shadow: 0 8px 32px rgba(0,0,0,0.15); display: none; z-index: 200; }
        .user-dropdown.show { display: block; }
        .user-dropdown a { display: flex; align-items: center; gap: 9px; padding: 9px 12px; border-radius: 7px; font-size: 0.85rem; color: var(--text); text-decoration: none; font-weight: 500; transition: background 0.12s; }
        .user-dropdown a:hover { background: var(--purple-light); color: var(--purple); }
        .dd-sep { height: 1px; background: var(--border); margin: 4px 0; }
        .logout-link { color: #CC0000 !important; }
        .logout-link:hover { background: #FFF3F3 !important; color: #CC0000 !important; }

        /* HERO */
        .hero { background: linear-gradient(145deg, var(--purple-deep) 0%, #3A1A7A 55%, #5B2DC5 100%); padding: 80px 80px 0; display: flex; align-items: flex-end; gap: 60px; min-height: 460px; overflow: hidden; position: relative; }
        .hero::before { content: ''; position: absolute; width: 500px; height: 500px; border-radius: 50%; background: rgba(212,168,67,0.05); top: -180px; right: -100px; }
        .hero-content { flex: 1; padding-bottom: 64px; position: relative; z-index: 1; }
        .hero-eyebrow { display: inline-flex; align-items: center; gap: 7px; background: rgba(212,168,67,0.15); border: 1px solid rgba(212,168,67,0.3); color: var(--gold); font-size: 0.72rem; font-weight: 700; padding: 5px 13px; border-radius: 20px; margin-bottom: 18px; letter-spacing: 1px; text-transform: uppercase; }
        .hero h1 { font-family: 'Playfair Display', serif; font-size: 3rem; font-weight: 700; color: #fff; line-height: 1.15; margin-bottom: 16px; }
        .hero h1 em { font-style: normal; color: var(--gold); }
        .hero p { font-size: 1rem; color: rgba(255,255,255,0.75); max-width: 460px; line-height: 1.7; margin-bottom: 32px; }
        .hero-btns { display: flex; gap: 12px; }
        .btn-gold { background: var(--gold); color: var(--purple-deep); font-weight: 700; font-size: 0.9rem; padding: 12px 28px; border-radius: 8px; border: none; cursor: pointer; transition: opacity 0.15s, transform 0.1s; box-shadow: 0 4px 14px rgba(212,168,67,0.4); font-family: 'DM Sans', sans-serif; }
        .btn-gold:hover { opacity: 0.9; transform: translateY(-1px); }
        .btn-outline-w { background: transparent; color: #fff; font-weight: 600; font-size: 0.9rem; padding: 12px 24px; border-radius: 8px; border: 1.5px solid rgba(255,255,255,0.35); cursor: pointer; transition: border-color 0.15s; font-family: 'DM Sans', sans-serif; }
        .btn-outline-w:hover { border-color: #fff; }
        .hero-right { flex: 0 0 340px; position: relative; z-index: 1; }
        .hero-cards { background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.12); border-radius: 14px 14px 0 0; padding: 18px; backdrop-filter: blur(10px); }
        .mini-card { background: #fff; border-radius: 10px; padding: 12px 14px; margin-bottom: 9px; display: flex; align-items: center; gap: 12px; box-shadow: 0 4px 14px rgba(0,0,0,0.1); }
        .mini-card:last-child { margin-bottom: 0; }
        .mini-icon { width: 36px; height: 36px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 1rem; flex-shrink: 0; }
        .ic1{background:linear-gradient(135deg,#6C3FC5,#9B72E8);}
        .ic2{background:linear-gradient(135deg,#D4A843,#F5CC6A);}
        .ic3{background:linear-gradient(135deg,#1B5E20,#388E3C);}
        .mini-info h4 { font-size: 0.78rem; font-weight: 700; margin-bottom: 2px; }
        .mini-info p { font-size: 0.68rem; color: var(--muted); }
        .mstars { color: var(--gold); font-size: 0.68rem; }

        /* SECTIONS */
        section { padding: 64px 80px; }
        .sec-label { font-size: 0.72rem; font-weight: 700; text-transform: uppercase; letter-spacing: 2px; color: var(--purple); margin-bottom: 6px; }
        .sec-title { font-family: 'Playfair Display', serif; font-size: 1.9rem; font-weight: 700; margin-bottom: 6px; }
        .sec-sub { font-size: 0.9rem; color: var(--muted); margin-bottom: 36px; }

        .cat-grid { display: grid; grid-template-columns: repeat(4,1fr); gap: 14px; }
        .cat-card { border: 1.5px solid var(--border); border-radius: 12px; padding: 22px 18px; text-decoration: none; color: var(--text); transition: all 0.2s; display: block; background: #fff; }
        .cat-card:hover { border-color: var(--purple); box-shadow: 0 6px 20px rgba(108,63,197,0.1); transform: translateY(-2px); color: var(--text); }
        .cat-icon { font-size: 1.8rem; margin-bottom: 10px; }
        .cat-card h3 { font-size: 0.9rem; font-weight: 700; margin-bottom: 3px; }
        .cat-card p { font-size: 0.75rem; color: var(--muted); }

        .course-grid { display: grid; grid-template-columns: repeat(4,1fr); gap: 18px; }
        .course-card { background: #fff; border: 1px solid var(--border); border-radius: 12px; overflow: hidden; text-decoration: none; color: var(--text); transition: all 0.2s; display: block; }
        .course-card:hover { box-shadow: 0 8px 28px rgba(108,63,197,0.12); transform: translateY(-3px); color: var(--text); }
        .course-thumb { height: 130px; display: flex; align-items: center; justify-content: center; font-size: 2.5rem; }
        ...
        .course-org { font-size: 0.68rem; font-weight: 700; color: var(--purple); text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 5px; }
        .course-body h3 { font-size: 0.85rem; font-weight: 700; line-height: 1.4; margin-bottom: 8px; }
        .course-meta { display: flex; align-items: center; gap: 5px; font-size: 0.75rem; }
        .c-stars{color:var(--gold);} .c-score{font-weight:700;} .c-count{color:var(--muted);}
        .c-tag { display: inline-block; background: var(--purple-light); color: var(--purple); font-size: 0.65rem; font-weight: 700; padding: 2px 8px; border-radius: 4px; margin-top: 8px; }

        .stats-bar { background: linear-gradient(135deg, var(--purple-deep), #3A1A7A); padding: 48px 80px; display: grid; grid-template-columns: repeat(4,1fr); gap: 24px; text-align: center; }
        .stat-num { font-family: 'Playfair Display', serif; font-size: 2.4rem; font-weight: 700; color: var(--gold); display: block; }
        .stat-lbl { font-size: 0.82rem; color: rgba(255,255,255,0.65); }

        .cta-sec { background: linear-gradient(135deg, #1E0A4A, var(--purple)); padding: 72px 80px; text-align: center; }
        .cta-sec h2 { font-family: 'Playfair Display', serif; font-size: 2.2rem; color: #fff; margin-bottom: 12px; }
        .cta-sec p { font-size: 0.95rem; color: rgba(255,255,255,0.7); margin-bottom: 32px; }

        footer { background: var(--purple-deep); padding: 40px 80px 20px; }
        .footer-grid { display: grid; grid-template-columns: 2fr 1fr 1fr 1fr; gap: 40px; margin-bottom: 32px; }
        .f-brand { font-family: 'Playfair Display', serif; font-size: 1.4rem; font-weight: 700; color: #fff; display: block; margin-bottom: 8px; }
        .f-brand span { color: var(--gold); }
        .f-desc { font-size: 0.82rem; color: rgba(255,255,255,0.45); line-height: 1.7; }
        .f-col h4 { font-size: 0.75rem; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; color: rgba(255,255,255,0.45); margin-bottom: 14px; }
        .f-col a { display: block; font-size: 0.82rem; color: rgba(255,255,255,0.6); text-decoration: none; margin-bottom: 8px; transition: color 0.15s; }
        .f-col a:hover { color: var(--gold); }
        .footer-bottom { border-top: 1px solid rgba(255,255,255,0.08); padding-top: 16px; font-size: 0.75rem; color: rgba(255,255,255,0.3); display: flex; justify-content: space-between; }
        ...
        .modal-overlay.show .auth-modal { transform: translateY(0) scale(1); opacity: 1; }
        .auth-modal::-webkit-scrollbar { width: 3px; }
        .auth-modal::-webkit-scrollbar-thumb { background: var(--border); border-radius: 2px; }
        ...
        .modal-close {
            ...
            display: flex; align-items: center; justify-content: center;
            font-size: 0.95rem; color: var(--muted); transition: background 0.15s;
        }
        .modal-close:hover { background: var(--border); color: var(--text); }

        .modal-title { font-family: 'Playfair Display', serif; font-size: 1.45rem; font-weight: 700; color: var(--text); margin-bottom: 3px; }
        .modal-sub   { font-size: 0.82rem; color: var(--muted); margin-bottom: 20px; }

        .tab-switch { display: flex; background: var(--purple-light); border-radius: 10px; padding: 4px; gap: 4px; margin-bottom: 20px; }
        .tab-btn { flex: 1; padding: 9px; border-radius: 7px; font-size: 0.875rem; font-weight: 600; border: none; background: transparent; color: var(--muted); cursor: pointer; transition: all 0.2s; font-family: 'DM Sans', sans-serif; }
        .tab-btn.active { background: var(--purple); color: #fff; box-shadow: 0 2px 8px rgba(108,63,197,0.3); }

        .form-panel { display: none; }
        .form-panel.active { display: block; }

        .f-label { font-size: 0.75rem; font-weight: 700; color: var(--text); margin-bottom: 5px; text-transform: uppercase; letter-spacing: 0.5px; display: block; }
        .f-input { width: 100%; border: 1.5px solid var(--border); border-radius: 9px; padding: 10px 13px; font-size: 0.875rem; font-family: 'DM Sans', sans-serif; color: var(--text); background: #FDFCFF; transition: border-color 0.15s, box-shadow 0.15s; outline: none; }
        .f-input::placeholder { color: #C4BAD9; }
        .f-input:focus { border-color: var(--purple); box-shadow: 0 0 0 3px rgba(108,63,197,0.1); background: #fff; }
        .pw-wrap { position: relative; }
        .pw-wrap .f-input { padding-right: 38px; }
        .pw-eye { position: absolute; right: 11px; top: 50%; transform: translateY(-50%); background: none; border: none; padding: 0; color: var(--muted); font-size: 0.875rem; cursor: pointer; }
        .pw-eye:hover { color: var(--purple); }
        .forgot { font-size: 0.75rem; font-weight: 600; color: var(--purple); text-decoration: none; }
        .forgot:hover { text-decoration: underline; }

        .btn-submit { width: 100%; background: linear-gradient(135deg, var(--purple), var(--purple-dark)); color: #fff; font-weight: 700; font-size: 0.875rem; padding: 12px; border: none; border-radius: 9px; cursor: pointer; transition: opacity 0.15s, transform 0.1s; box-shadow: 0 4px 14px rgba(108,63,197,0.35); display: flex; align-items: center; justify-content: center; gap: 7px; font-family: 'DM Sans', sans-serif; }
        .btn-submit:hover { opacity: 0.9; transform: translateY(-1px); }

        .or-div { display: flex; align-items: center; gap: 10px; font-size: 0.72rem; color: var(--muted); margin: 12px 0; }
        ...
        .btn-soc { border: 1.5px solid var(--border); background: #fff; color: var(--text); font-weight: 600; font-size: 0.78rem; padding: 9px 10px; border-radius: 9px; display: flex; align-items: center; justify-content: center; gap: 7px; cursor: pointer; transition: all 0.15s; font-family: 'DM Sans', sans-serif; text-decoration: none; }
        .btn-soc:hover { border-color: var(--purple); background: var(--purple-light); color: var(--text); }

        .name-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
        .role-grid { display: grid; grid-template-columns: repeat(3,1fr); gap: 7px; }
        .role-opt input { display: none; }
        .role-opt label { border: 1.5px solid var(--border); border-radius: 8px; padding: 9px 4px; text-align: center; cursor: pointer; font-size: 0.68rem; font-weight: 600; color: var(--muted); display: block; transition: all 0.15s; }
        .role-opt label .ri { font-size: 1.1rem; display: block; margin-bottom: 2px; }
        .role-opt input:checked + label { border-color: var(--purple); background: var(--purple-light); color: var(--purple); }
        ...
        .str-lbl { font-size: 0.67rem; color: var(--muted); margin-top: 2px; }

        .alert-err { background: #FFF3F3; border: 1px solid #FFB3B3; color: #CC0000; border-radius: 8px; padding: 9px 12px; font-size: 0.78rem; margin-bottom: 14px; display: flex; align-items: center; gap: 7px; }
        .form-check-input:checked { background-color: var(--purple); border-color: var(--purple); }

        @media(max-width:900px){
            .hero{flex-direction:column;padding:48px 24px 0;} .hero-right{display:none;}
            section,.stats-bar,.cta-sec,footer{padding:48px 24px;}
            .cat-grid,.course-grid{grid-template-columns:repeat(2,1fr);}
            .stats-bar,.footer-grid{grid-template-columns:repeat(2,1fr);}
...
<script>
    ...
    document.getElementById('rCpw').addEventListener('input', function() {
        this.style.borderColor=''; document.getElementById('cpwErr').textContent='';
    });
