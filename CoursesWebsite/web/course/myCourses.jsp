    ...
    <head>
        ...
        <style>
            ...
            *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
            ...
            .mobile-nav a { color: rgba(255,255,255,0.85); text-decoration: none; font-size: 1.1rem; font-weight: 600; padding: 16px 0; border-bottom: 1px solid rgba(255,255,255,0.08); display: flex; align-items: center; gap: 12px; transition: color 0.15s; }
            ...
            .course-card:nth-child(1){animation-delay:0.05s} .course-card:nth-child(2){animation-delay:0.1s} .course-card:nth-child(3){animation-delay:0.15s}
            .course-card:nth-child(4){animation-delay:0.2s}  .course-card:nth-child(5){animation-delay:0.25s} .course-card:nth-child(6){animation-delay:0.3s}
            ...
            .card-thumb { height: 158px; position: relative; display: flex; align-items: center; justify-content: center; overflow: hidden; }
            ...
            .bg1{background:linear-gradient(135deg,#1E0A4A,#6C3FC5)} .bg2{background:linear-gradient(135deg,#3A1A7A,#9B72E8)}
            .bg3{background:linear-gradient(135deg,#4E2C96,#D4A843)} .bg4{background:linear-gradient(135deg,#1A0D35,#5B2DC5)}
            .bg5{background:linear-gradient(135deg,#0D47A1,#1565C0)} .bg6{background:linear-gradient(135deg,#1B5E20,#388E3C)}
            ...
            /* ================================================================
               RESPONSIVE – theo pattern homePage.jsp
               ================================================================ */
            ...
            @media (max-width: 900px) {
                ...
                .balance-pill { display: none; }           /* ẩn hoàn toàn như homePage */
            ...
            @media (max-width: 600px) {
                ...
                .page-header  { padding: 24px 16px 28px; }
                .page-title   { font-size: 1.75rem; }
                .page-subtitle{ font-size: 0.875rem; }
                .header-stats { gap: 18px; margin-top: 18px; flex-wrap: wrap; }
                .stat-num     { font-size: 1.2rem; }

                .main-content { padding: 20px 16px 40px; }

                /* Tabs scroll ngang */
                .tab-bar { overflow-x: auto; -webkit-overflow-scrolling: touch; scrollbar-width: none; padding-bottom: 4px; }
                .tab-bar::-webkit-scrollbar { display: none; }
                .tab-btn { flex-shrink: 0; font-size: 0.78rem; padding: 8px 14px; }

                .course-grid  { grid-template-columns: 1fr; gap: 14px; }
                .card-thumb   { height: 180px; }

                .card-footer  { flex-wrap: wrap; gap: 10px; }
                .btn-study, .btn-cert, .btn-review { flex: 1; justify-content: center; font-size: 0.78rem; padding: 8px 12px; }

                .empty-state  { padding: 60px 16px; }
                .empty-icon-wrap { width: 72px; height: 72px; font-size: 2.2rem; }
    ...
    <body>
        ...
        <c:if test="${not empty MY_COURSES}">
            <c:forEach var="course" items="${MY_COURSES}">
                <c:choose>
                    <c:when test="${course.status eq '2'}"><c:set var="completedCount" value="${completedCount + 1}"/></c:when>
                    <c:otherwise><c:set var="inProgressCount" value="${inProgressCount + 1}"/></c:otherwise>
                </c:choose>
            </c:forEach>
        </c:if>
        ...
        <div class="mobile-nav" id="mobileNav">
            ...
            <c:if test="${sessionScope.user.role == 1}">
                <a href="${pageContext.request.contextPath}/adminController?action=dashboard"><i class="bi bi-shield-fill"></i> Administrator Manager</a>
            </c:if>
            ...
            <a href="${pageContext.request.contextPath}/user/myprofile.jsp"><i class="bi bi-person-fill"></i> Hồ sơ của tôi</a>
            ...
            <a href="${pageContext.request.contextPath}/wishlistController?action=view"><i class="bi bi-heart-fill"></i> Yêu thích</a>
            <div class="mobile-nav-divider"></div>
        ...
        <nav class="navbar-main" style="position:relative;">
            ...
            <ul class="nav-links">
                ...
                <li><a href="${pageContext.request.contextPath}/course/myCourses">Giảng viên</a></li>
                <c:if test="${sessionScope.user.role == 1}">
            ...
            <div class="nav-right">
                ...
                <a href="${pageContext.request.contextPath}/mainController?action=payment" class="balance-pill">
                    ...
                    <span class="balance-amount">
                        <fmt:formatNumber value="${sessionScope.user.balance != null ? sessionScope.user.balance : 0}" type="number" maxFractionDigits="0"/> ₫
                    </span>
                </a>
                                ...
                                <c:when test="${not empty WISHLIST_COURSES}">
                                    <c:forEach var="wc" items="${WISHLIST_COURSES}">
                                        <div class="wishlist-dd-item" id="wish-item-${wc.courseId}">
                                            <div class="wishlist-dd-thumb">
                                                <c:choose>
                                                    <c:when test="${not empty wc.img}">
                                                        <img src="${pageContext.request.contextPath}/${wc.img}" alt="${wc.courseName}" onerror="this.style.display='none'; this.parentNode.innerHTML='📚';">
                                                    </c:when>
                                                    <c:otherwise>
                                                        <img src="${pageContext.request.contextPath}/img/courses/course${wc.courseId}.jpg" alt="${wc.courseName}" onerror="this.style.display='none'; this.parentNode.innerHTML='📚';">
                                                    </c:otherwise>
                                                </c:choose>
                                            </div>
                                            ...
                                            <button class="wishlist-dd-remove" title="Xóa khỏi yêu thích" onclick="removeWishItem(event, '${wc.courseId}')">
                                                <i class="bi bi-x"></i>
                                            </button>
                                        </div>
                                    </c:forEach>
        ...
        <div class="page-header">
            <div class="page-header-inner">
                ...
                <div class="header-stats">
                    <div><div class="stat-num">${not empty MY_COURSES ? fn:length(MY_COURSES) : 0}</div><div class="stat-lbl">Khóa đã đăng ký</div></div>
                    <div><div class="stat-num">${inProgressCount}</div><div class="stat-lbl">Đang học</div></div>
                    <div><div class="stat-num">${completedCount}</div><div class="stat-lbl">Đã hoàn thành</div></div>
                </div>
            </div>
        </div>
            ...
            <c:choose>
                <c:when test="${not empty MY_COURSES}">
                    <div class="tab-bar">
                        <button class="tab-btn active" onclick="filterTab(this, 'all')">
                            <i class="bi bi-collection-play-fill"></i> Tất cả <span class="tab-count">${fn:length(MY_COURSES)}</span>
                        </button>
                        <button class="tab-btn" onclick="filterTab(this, 'progress')">
                            <i class="bi bi-play-circle-fill"></i> Đang học <span class="tab-count">${inProgressCount}</span>
                        </button>
                        <button class="tab-btn" onclick="filterTab(this, 'done')">
                            <i class="bi bi-trophy-fill"></i> Đã hoàn thành <span class="tab-count">${completedCount}</span>
                        </button>
                    </div>
                            ...
                            <div class="course-card ${isDone ? 'completed' : ''}" data-status="${isDone ? 'done' : 'progress'}">
                                <div class="card-thumb bg${(st.index % 6) + 1}">
                                    <c:if test="${not empty course.img}">
                                        <img src="${pageContext.request.contextPath}/${course.img}" alt="${course.courseName}" onerror="this.style.display='none';">
                                    </c:if>
                                    <div class="card-thumb-overlay"></div>
                                    <span class="card-topic-badge">${course.topic}</span>
                                    <c:if test="${isDone}">
                                        <span class="card-completed-badge"><i class="bi bi-trophy-fill"></i> Hoàn thành</span>
                                    </c:if>
                                    <div class="card-progress-wrap">
                                        <div class="card-progress-fill ${isDone ? 'full' : ''}" style="${isDone ? '' : 'width:40%;'}"></div>
                                    </div>
                                </div>
                                <div class="card-body">
                                ...
                                <div class="card-footer">
                                    <c:choose>
                                        <c:when test="${isDone}">
                                            <span class="card-fee done"><i class="bi bi-trophy-fill"></i> Hoàn thành</span>
                                        </c:when>
                                        <c:otherwise>
                                            <span class="card-fee ${course.fee == 0 ? 'free' : ''}">
                                                ...
                                                <c:choose>
                                                    ...
                                                    <c:otherwise>${course.fee} ₫</c:otherwise>
                                                </c:choose>
                                            </span>
                                        </c:otherwise>
                                    </c:choose>
                                    <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
                                        <a href="${pageContext.request.contextPath}/courseController?action=lesson&courseId=${course.courseId}"
                                           class="${isDone ? 'btn-review' : 'btn-study'}">
                                            <i class="bi bi-${isDone ? 'arrow-repeat' : 'play-circle-fill'}"></i>
                                            ${isDone ? 'Học lại' : 'Vào học'}
                                        </a>
                                        <c:if test="${isDone}">
                                            <a href="${pageContext.request.contextPath}/mainController?action=certificate&courseId=${course.courseId}" class="btn-cert">
                ...
                <c:otherwise>
                    <div class="empty-state">
                        ...
                        <p class="empty-sub">Bạn chưa đăng ký khóa học nào. Hãy khám phá thư viện và bắt đầu hành trình học tập!</p>
                        <a href="${pageContext.request.contextPath}/mainController?action=ExploreCourse" class="btn-explore">
        ...
        <script src="${pageContext.request.contextPath}/assets/js/mycourse.js" type="text/javascript"></script>
        <script>
            ...
            document.addEventListener('click', function(e) {
                ...
                if (userDD && userMenu && !userMenu.contains(e.target) && !userDD.contains(e.target)) userDD.classList.remove('show');
                if (wDD && wWrap && !wWrap.contains(e.target)) wDD.classList.remove('show');
